...
`results/hw1/roofline.png` + `roofline_data.json` with correctly-placed points.
...
- [ ] **2A.1** Use `profiler.profile` with `ProfilerActivity.CPU` + `.CUDA`.
- [ ] **2A.2** Print `key_averages().table(sort_by="self_cuda_time_total")` (self CPU on CPU-only).
- [ ] **2A.3** `export_chrome_trace(trace_path)`; pass the self-check.
...
- [ ] **2B.1** Explain prefill vs decode and what `past_key_values` stores (PLAN §7).
- [ ] **2B.2** Prefill the prompt with `use_cache=True`; keep the returned cache.
- [ ] **2B.3** Decode loop: feed the **single** last token + cache; `argmax` the last logit.
- [ ] **2B.4** **No `.item()`** on the hot path; collect tokens on-device, concat at the end.
- [ ] **2B.5** Confirm exact-match correctness vs `baseline_loop` (fp32).
...
hits the tier (target **≥ 4×**) on GPU.

- [ ] **2C.1** Build the model (bf16 for the timed run); reuse `optimized_loop`.
- [ ] **2C.2** Measure with `time_loop` (warmed up); return `(seconds, ids)`.
- [ ] **2C.3** Optionally layer `torch.compile` / `reduce-overhead`; re-measure.
- [ ] **2C.4** Confirm the speedup tier and that correctness still passes.
- **Learn:** bf16 tradeoffs; warmup vs steady state; stacking optimizations.
...
- [ ] **2D.3** Q3: biggest single change and why for this workload.
- [ ] **2D.4** Q4: memory-traffic vs CPU/launch optimizations; which mattered more.
...
- [ ] **3A.1** `sweep_elementwise_times`: same out-of-place op over fp32 tensors per size, timed with `bench()`.
- [ ] **3A.2** `estimate_launch_overhead_us`: average the launch-bound sizes (`n ≤ 2**12`), → µs.
- [ ] **3A.3** Run the sweep; read the flat region in the plot.
...
- [ ] **3B.1** Run `dynamo.explain` on `broken_decode_step`; read the breaks.
- [ ] **3B.2** Keep `scale` a **0-dim tensor** (no `.item()`).
- [ ] **3B.3** Replace the `if` with `torch.where`; drop the `print`.
- [ ] **3B.4** Match the broken math (`allclose`); compile `fullgraph=True`; 0 breaks.
...
- [ ] **3C.1** Clone a `static_in`; warm up on a **side stream**.
- [ ] **3C.2** Capture once under `torch.cuda.graph(g)` → `static_out`.
- [ ] **3C.3** `graphed(x)`: `static_in.copy_(x)`, `g.replay()`, `return static_out.clone()`.
- [ ] **3C.4** Pass both self-checks; run the final benchmark (replay should win).
...
1. **Only write in `YOUR IMPLEMENTATION` cells.** Never touch `DO NOT EDIT`.
...
3. **Notebook order is a dependency:** finish HW1 → HW2 → HW3 (the concept stack,
...
# Inspect traces: upload results/hw2/trace_*.json to https://ui.perfetto.dev
```
